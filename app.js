...
const PUMPS = [
  ...
  { key: "sirkulasi", name: "Sirkulasi", pin: 14, group: "Sirkulasi", controlTopic: MQTT_CONFIG.topics.controlSirkulasi, statusTopic: MQTT_CONFIG.topics.statusSirkulasi },
...
const state = {
  ...
  history: { tds: [], temp: [], ph: [] },
  pumps: { aerator: false, sirkulasi: false, nutrisiA: false, nutrisiB: false, phUp: false, phDown: false },
  phMode: "auto",
    ...
    try {
        ...
        mqttClient.on('connect', () => {
            ...
            const topics = [
                ...
                MQTT_CONFIG.topics.statusSirkulasi,
    ...
    if (state.phMode === 'auto') {
        if (fuzzy.action === 'dosing-up') {
            ...
            if (!state.pumps.phDown) {
                ...
                showToast('📤 pH Down ON (Fuzzy)', 'info');
            }
            if (state.pumps.phUp) {
...
function publishControl(topic, value) {
    ...
    if (topic === MQTT_CONFIG.topics.controlPhMode) {
        ...
        if (value === state.lastModeSent && (now - state.modeSendTime) < 2000) {
            console.log('⏳ Skip duplicate mode send:', value);
            return true;
        }
        state.lastModeSent = value;
        state.modeSendTime = now;
    }
...
function renderPhPanel() {
  ...
  const presetOptions = ['<option value="custom">🎯 Target manual…</option>']
    .concat(PLANT_PRESETS.filter(p => p.key !== 'custom').map((p) => `<option value="${p.key}" ${state.preset === p.key ? "selected" : ""}>${p.name} (pH ${p.phMin}–${p.phMax})</option>`))
    .join("");
  ...
  panel.innerHTML = `
    ...
    <div class="ph-right">
      <div class="fuzzy-box">
        <h4>🧠 Fuzzy Logic · e = ${(ph - (min + max) / 2).toFixed(2)} ${manual ? '(Nonaktif)' : ''}</h4>
        ${fuzzyRows}
      </div>
...
function renderPumps() {
  ...
  const groups = {
    ...
    "Dosis Nutrisi": allPumps.filter(p => p.group === "Dosis Nutrisi"),
    "pH Control": PH_PUMPS
  };
  
  container.innerHTML = Object.keys(groups).filter(key => groups[key].length > 0).map((group) => {
        ...
        <div class="pump-row">
          <div class="pump-info">
            ...
            <div>
              ...
              <div class="pump-pin">GPIO ${p.pin} ${isPhPump && state.phMode === 'auto' ? '(Auto)' : ''}</div>
            </div>
          </div>
          <label class="switch">
            <input type="checkbox" ${on ? "checked" : ""} data-pump="${p.key}" ${disabled} aria-label="${on ? "Matikan" : "Nyalakan"} pompa ${p.name}" />
...
function togglePump(key, val) {
  state.pumps[key] = val;
  const allPumps = [...PUMPS, ...PH_PUMPS];
  const pump = allPumps.find(p => p.key === key);
  if (pump) {
