...
function handleMessage(topic, payload) {
    ...
    if (topic === TOPICS.all) {
        try {
            ...
            // ★★★ Jalankan Fuzzy Logic setiap ada data baru ★★★
...
function publish(topic, value) {
    if (!client || !client.connected) {
        toast('MQTT tidak terhubung!', 'warning');
        return false;
    }
    ...
    dom.pumpGrid.querySelectorAll('[data-pump]').forEach(el => {
        el.addEventListener('change', (e) => {
            ...
            if ((key === 'phUp' || key === 'phDown') && state.phMode === 'auto') {
                toast('Mode AUTO, pH dikontrol otomatis!', 'warning');
...
function renderPhPanel() {
    ...
    dom.phPanel.querySelectorAll('[data-dose]').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (state.phMode !== 'manual') {
                toast('Ganti ke mode manual dulu!', 'warning');
                return;
            }
    ...
    if (emergencyBtn) {
        emergencyBtn.addEventListener('click', () => {
            if (confirm('Yakin ingin mematikan SEMUA relay?')) {
